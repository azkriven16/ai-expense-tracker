...
export async function generateAIAnswer(
  ...
  try {
    const totalSpent = context.reduce((sum, exp) => sum + exp.amount, 0);
    const categoryBreakdown = context.reduce((acc, exp) => {
    ...
    const contextSummary = {
      ...
      recentExpenses: context.slice(0, 5),
    };

    const prompt = `Based on this expense data, answer the question concisely and actionably.
...
2. Uses specific data from the expenses
3. Offers 1-2 actionable suggestions
4. Keeps response under 200 characters
...
Answer:`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful financial advisor. Be concise, specific, and actionable. Use data from the context.",
        },
        {
          role: "user",
          content: prompt,
    ...
    return "I'm unable to provide a detailed answer right now. Please try again or refresh your insights.";
  }
}
